...
struct BtorWorker
{
	...
	void export_cell(Cell *cell)
	{
		...
		if (cell->type.isPublic()) {
			Module *submodule = module->design->module(cell->type);

			SigSpec input_signals;
			SigSpec output_signals;
			...
			btorf("%d instance %s %d\n", next_nid, log_id(cell->type), inp_nid);
			add_nid_sig(next_nid++, output_signals);
			goto okay;
		}
			...
			f(f), sigmap(module), module(module), verbose(verbose), single_bad(single_bad), cover_mode(cover_mode), print_internal_names(print_internal_names), info_filename(info_filename)
	{
		...
		for (auto port_id : module->ports)
		{
			Wire *wire = module->wire(port_id);
	...
	void execute(std::ostream *&f, std::string filename, std::vector<std::string> args, RTLIL::Design *design) override
	{
		...
		for (auto module : design->selected_modules()) {
			*f << stringf("; BTOR description generated by %s for module %s.\n",
				yosys_maybe_version(), log_id(module));
			*f << stringf("0 %s\n", log_id(module->name));

			BtorWorker(*f, module, verbose, single_bad, cover_mode, print_internal_names, info_filename, ywmap_filename);
